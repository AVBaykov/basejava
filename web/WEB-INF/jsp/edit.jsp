            ...
            <c:choose>
                ...
                <c:when test="${type=='QUALIFICATIONS' || type=='ACHIEVEMENT'}">
                    <textarea name='${type}' cols=75
                              rows=5><%=String.join("\n", ((ParagraphSection) section).getParagraphList())%></textarea>
                </c:when>
                <c:when test="${type=='EXPERIENCE' || type=='EDUCATION'}">
                    <c:forEach var="org" items="<%=((PlaceSection) section).getPlaces()%>"
                               varStatus="counter">
                        <dl>
                        ...
                        <div style="margin-left: 30px">
                            <c:forEach var="pos" items="${org.periodList}">
                                <jsp:useBean id="pos" type="ru.javawebinar.basejava.model.Place.Period"/>
                                <dl>
